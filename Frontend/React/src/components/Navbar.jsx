...
import {
  ...
  ChevronDown,
  X,
} from "lucide-react"; // Changed Bookmark -> Library
...
export default function Navbar() {
  ...
  const navBtnBase =
    "inline-flex items-center justify-center whitespace-nowrap rounded-md font-medium transition-all duration-200 h-9 px-2 sm:px-4 py-2 text-xs sm:text-sm focus:outline-none active:scale-95";

  const getBtnClass = (route) =>
    isActive(route)
      ? "bg-zinc-800 text-white shadow-sm ring-1 ring-white/10"
      : "text-zinc-400 hover:bg-zinc-800/50 hover:text-white";

  const glassStyle =
    "bg-zinc-900/80 backdrop-blur-xl border border-white/10 shadow-lg shadow-black/30";

  return (
    <>
      <div className="fixed top-2 sm:top-4 left-0 right-0 z-50 flex flex-col items-center px-2 sm:px-4 pointer-events-none">
        {/* --- NAVBAR --- */}
        <nav className="relative pointer-events-auto flex w-full max-w-7xl items-center justify-between gap-1 sm:gap-2 rounded-3xl border border-zinc-800 bg-zinc-950/95 backdrop-blur p-2 shadow-2xl z-20">
          {/* LOGO */}
          <Link
            to="/"
          ...
          <div className="flex items-center gap-1 sm:gap-1 flex-shrink-0">
            ...
            <Link
              to="/categories"
              className={`${navBtnBase} ${getBtnClass("/categories")}`}
            >
            ...
            {user ? (
              <Link
                to="/saved"
                className={`${navBtnBase} ${getBtnClass("/saved")}`}
              >
            ...
            {user && (
              <div className="relative" ref={profileRef}>
                ...
                >
                  ...
                  <span
                    className={`hidden lg:inline text-sm font-semibold max-w-[100px] truncate ${isActive("/profile") ? "text-white" : "text-zinc-200"}`}
                  >
                    {user.username}
                  </span>
                  <ChevronDown
                    className={`hidden lg:block h-3 w-3 transition-transform duration-200 ${isProfileOpen ? "rotate-180" : ""}`}
                  />
                </button>
        ...
        {showSearch && (
          <div className="md:hidden w-full max-w-md mt-4 relative z-10 pointer-events-auto flex items-center justify-center gap-3">
            ...
            >
              ...
              <span
                className={`font-medium text-zinc-300 text-sm whitespace-nowrap overflow-hidden transition-all duration-300 ${isSearchOpen ? "w-0 opacity-0 ml-0" : "w-auto opacity-100 ml-2"}`}
              >
                Cerca
              </span>
            ...
            <div
              className={`transition-all duration-500 cubic-bezier(0.32, 0.72, 0, 1) overflow-hidden ${isSearchOpen ? "w-12 opacity-100 scale-100" : "w-0 opacity-0 scale-0"}`}
            >
              <button
