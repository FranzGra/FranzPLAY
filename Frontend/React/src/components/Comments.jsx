  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        alert(
          res.message || res.messaggio || "Errore sconosciuto durante l'invio.",
        );
      }
    } catch (error) {
  ...
  return (
    <div className="bg-zinc-900 backdrop-blur-xl border border-white/10 shadow-xl rounded-xl p-4 md:p-6 h-full flex flex-col">
          ...
          comments.map((c) => (
            ...
            >
              <div className="flex-shrink-0 h-8 w-8 md:h-10 md:w-10 rounded-full bg-zinc-800 border border-white/10 overflow-hidden">
                {c.Immagine_Profilo ? (
                  <img
                    src={getUserAvatarUrl(c.Immagine_Profilo)}
                    alt=""
              ...
              <div className="flex-1 bg-zinc-800 p-3 rounded-2xl rounded-tl-none border border-white/10">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-[10px] text-zinc-500">
                    {new Date(c.data_ora_commento).toLocaleDateString()}
                  </span>
                </div>
                <p className="text-zinc-100 text-sm mt-1 whitespace-pre-wrap">
                  {c.testo_commento}
                </p>
      ...
      <form
        ...
        className="mt-4 flex gap-2 pt-4 border-t border-white/10 relative"
      >
