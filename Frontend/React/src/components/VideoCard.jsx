...
function VideoCard({ video, onRemove, RemoveIcon = X }) {
  ...
  if (video.progresso_secondi && video.Durata) {
    ...
    if (totalSeconds > 0) {
      progressPercent = Math.min(
        Math.max((video.progresso_secondi / totalSeconds) * 100, 0),
        100,
      );
    }
  }
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("stop-other-previews", handleStopOthers);
  }, [video.id]);
  ...
  const handleTouchStart = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("stop-other-previews", { detail: { id: video.id } }),
    );
  };
  ...
  return (
    <div
      className={`relative group block h-full transition-all duration-300 ease-out ${isRemoving ? "opacity-0 scale-90 pointer-events-none" : "opacity-100 scale-100"}`}
    >
      <Link
        to={`/watch/${video.id}`}
      ...
      >
        ...
        <div className="relative aspect-video w-full rounded-xl bg-zinc-900 overflow-hidden shadow-lg ring-1 ring-white/10 group-hover:ring-zinc-500/50 transition-all mb-3 flex-shrink-0">
          ...
          {onRemove ? (
            ...
            >
              <RemoveIcon
                size={14}
                className="text-white flex-shrink-0 md:w-4 md:h-4"
              />
        ...
        <div className="flex flex-col gap-1 px-1 flex-1">
          <h3
            className="text-md font-bold text-white leading-tight transition-colors line-clamp-2 mb-auto"
            title={video.Titolo}
          >
            {video.Titolo}
          </h3>
