...
export default function Player() {
  ...
  const handleLike = async () => {
    ...
    try {
      await apiRequest("/toggleLike.php", "POST", { videoId: id }, false);
    } catch (e) {
  ...
  const handleSave = async () => {
    ...
    try {
      await apiRequest("/toggleSalvati.php", "POST", { videoId: id }, false);
    } catch (e) {
      setIsSaved(!newSavedState);
    }
  };
  ...
  if (!video) {
    return (
      <div className="h-screen w-full bg-zinc-950 flex flex-col items-center justify-center text-zinc-500 gap-3">
        ...
        <button
          onClick={() => navigate("/")}
          className="mt-4 px-4 py-2 bg-zinc-800 rounded-lg hover:bg-zinc-700 text-white transition"
        >
          Torna alla Home
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-4 text-zinc-400 text-sm font-medium">
              ...
              <span className="flex items-center gap-1.5">
                <Calendar size={16} />{" "}
                {new Date(video.data_Pubblicazione).toLocaleDateString()}
              </span>
              {/* Format Badge */}
              {video.Formato && (
          ...
          <div className="flex flex-wrap items-center gap-3 pb-2">
            <button
              onClick={handleLike}
              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-full font-medium transition-all active:scale-95 whitespace-nowrap ${isLiked ? "bg-[var(--primary-color)] text-white shadow-lg shadow-[var(--primary-color)]/20" : "bg-zinc-800 text-white hover:bg-zinc-700 border border-zinc-700/50"}`}
            >
              <ThumbsUp size={20} className={isLiked ? "fill-white" : ""} />
              <span>Mi piace</span>
              <span
                className={`text-xs ml-1 py-0.5 px-1.5 rounded-md ${isLiked ? "bg-black/20" : "bg-white/10"}`}
              >
                {likes}
              </span>
            </button>
            ...
            <button
              onClick={handleSave}
              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-full font-medium transition-all active:scale-95 border whitespace-nowrap ${isSaved ? "bg-zinc-800 border-[var(--primary-color)]/50 text-[var(--primary-color)]" : "bg-zinc-800 border-zinc-700/50 text-white hover:bg-zinc-700"}`}
            >
              <Bookmark
                size={20}
                className={isSaved ? "fill-[var(--primary-color)]" : ""}
              />
            ...
            <button
              onClick={handleShare}
              className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 rounded-full font-medium transition-all active:scale-95 border whitespace-nowrap ${isCopied ? "bg-green-600 border-green-500 text-white" : "bg-zinc-800 border-zinc-700/50 text-white hover:bg-zinc-700"}`}
            >
