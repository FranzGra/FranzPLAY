...
export default function Profile() {
  ...
  return (
    <main className="pt-24 md:pt-32 pb-10 w-full px-4 md:px-0 md:max-w-[90%] xl:max-w-[80%] mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row items-center gap-6 border-b border-zinc-800 pb-8">
        ...
        <div className="relative group flex-shrink-0">
          <div className="h-24 w-24 md:h-32 md:w-32 rounded-full bg-zinc-800 overflow-hidden ring-4 ring-zinc-900 shadow-2xl">
            {user?.avatar ? (
              <img
                src={`${getUserAvatarUrl(user.avatar)}?t=${Date.now()}`}
                alt="Avatar"
          ...
          <button
            ...
            className="absolute inset-0 bg-black/60 rounded-full flex items-center justify-center opacity-0 md:group-hover:opacity-100 transition-opacity cursor-pointer backdrop-blur-[2px]"
          >
            ...
            <div className="flex justify-center md:block">
              ...
              >
                ...
                <div className="text-left md:text-center">
                  <div className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 group-hover:text-[var(--primary-color)] transition-colors leading-none mb-1">
                    Accesso
                  </div>
      ...
      {message ? (
        <div
          className={`p-4 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2 shadow-lg ${message.type === "success" ? "bg-green-950/50 text-green-400 border border-green-800" : "bg-red-950/50 text-red-400 border border-red-800"}`}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8">
        <div className="space-y-2">
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === "themes" ? "bg-zinc-800 text-white shadow-lg ring-1 ring-white/5" : "text-zinc-400 hover:bg-zinc-800/50 hover:text-white"}`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === "security" ? "bg-zinc-800 text-white shadow-lg ring-1 ring-white/5" : "text-zinc-400 hover:bg-zinc-800/50 hover:text-white"}`}
          >
            <Lock className="h-5 w-5" /> Sicurezza
          </button>
          <button
            onClick={() => setActiveTab("danger")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all font-medium ${activeTab === "danger" ? "bg-red-950/20 text-red-400 border border-red-900/30 shadow-lg" : "text-zinc-400 hover:bg-zinc-800/50 hover:text-white"}`}
          >
