...
function SubBadge({ lingua, stato }) {
  ...
  return (
    <Badge variant="outline" title={`${LABEL_LINGUA[lingua] || lingua} — ${st.label}`} className={`gap-1 px-2 py-0.5 text-xs uppercase font-bold tracking-widest ${st.border} ${st.bg} ${st.color}`}>
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-8 relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex bg-surface-2 p-1.5 rounded-2xl border border-hairline backdrop-blur-md self-start md:self-auto">
          <Button
          ...
          >
            ...
            {queueCount > 0 && (
              <span className="ml-1 inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full bg-amber-500 text-black text-xs font-black">
                {queueCount}
              </span>
            )}
          </Button>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center gap-3 justify-end">
        <div className="flex bg-surface-2 p-1.5 rounded-2xl border border-hairline backdrop-blur-md mr-auto">
        ...
        <div className="flex bg-surface-2 p-1.5 rounded-2xl border border-hairline backdrop-blur-md">
        ...
        <div className="relative min-w-[150px] z-20">
          <select
            ...
            className="w-full bg-surface-2 border border-hairline rounded-2xl py-2.5 px-4 text-foreground focus:outline-none focus:border-primary/50 transition-all font-bold text-sm appearance-none cursor-pointer"
          >
        ...
        <div className="relative group w-full md:w-72">
          ...
          <Input
            ...
            className="pl-12 pr-4 h-11 bg-surface-2 border-hairline rounded-2xl focus-visible:ring-primary/50 text-foreground"
          />
        </div>
      </div>
      ...
      <div className="min-h-[400px]">
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => <Card key={i} className="aspect-video bg-surface-2 rounded-3xl animate-pulse border-hairline" />)}
          </div>
        ) : videos.length === 0 ? (
          <Card className="flex flex-col items-center justify-center py-20 bg-transparent border-dashed border-hairline-strong">
            <Captions size={48} className="text-muted-foreground mb-4" />
            ...
            {videos.map((video) => {
              ...
              return (
                <motion.div variants={itemVariants} key={video.id}>
                  <Card className="group relative transform-gpu hover:z-10 flex flex-col h-full gap-0 py-0 overflow-hidden bg-surface-1 hover:bg-surface-2/70 border-hairline hover:border-hairline-strong transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/40">
                    {/* Cover full-bleed con overlay durata e stato sottotitoli */}
                    <div data-slot="card-media" className="relative aspect-video overflow-hidden bg-background">
                      ...
                      {video.Durata && (
                        <div className="absolute bottom-2.5 right-2.5 flex items-center gap-1 px-2 py-0.5 rounded-md bg-black/70 backdrop-blur-sm text-white text-xs font-bold">
                          <Clock size={11} /> {video.Durata}
                        </div>
                      )}
                      <div className="absolute bottom-2.5 left-2.5">
                        {subs.length === 0 ? (
                          <Badge variant="outline" className="gap-1 bg-black/70 backdrop-blur-sm border-hairline-strong text-foreground text-xs">
                            <CircleSlash size={11} /> Nessun sub
                          </Badge>
                        ) : (
                          <Badge variant="outline" className="gap-1 bg-emerald-500/20 backdrop-blur-sm border-emerald-500/30 text-emerald-300 text-xs">
                    ...
                    <CardContent className="p-4 flex flex-col flex-1 gap-3">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 min-h-[24px]">
                        {subs.length > 0 ? (
                          ...
                          <span className="text-sm text-muted-foreground italic">Nessun sottotitolo generato</span>
                        )}
                      </div>

                      <div className="flex items-center justify-between gap-2 mt-auto pt-1">
                        <Badge variant="outline" className="text-xs bg-surface-3 border-hairline truncate max-w-full uppercase">
            ...
            <Table>
              <TableHeader className="bg-surface-2">
                <TableRow className="border-hairline hover:bg-transparent">
                  ...
                  <TableHead className="text-xs font-black uppercase tracking-widest text-muted-foreground">Sottotitoli</TableHead>
                    ...
                    <TableRow key={video.id} className="border-hairline group hover:bg-surface-2 transition-colors">
                      <TableCell className="font-medium">
                        <div className="flex items-center gap-4">
                          <div className="relative w-20 aspect-video rounded-lg overflow-hidden bg-background border border-hairline-strong shrink-0">
                      ...
                      <TableCell>
                        <div className="flex flex-wrap items-center gap-1.5 max-w-[280px]">
                          {subs.length === 0 ? (
                            <Badge variant="outline" className="gap-1 text-muted-foreground text-xs border-hairline-strong">
      ...
      <Dialog open={!!modalVideo} onOpenChange={(open) => !open && setModalVideo(null)}>
        <DialogContent className="sm:max-w-5xl! w-[calc(100%-2rem)] bg-background border-hairline-strong p-0 overflow-hidden shadow-2xl">
...
function SubtitleModalContent({ video, onClose, onChanged }) {
  ...
  return (
    <div className="flex flex-col h-full max-h-[90vh]">
      <DialogHeader className="px-6 py-4 border-b border-hairline bg-surface-2 shrink-0 flex flex-row items-center justify-between">
      ...
      <div className="overflow-y-auto p-6 grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
        <div>
          <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground mb-3">Sottotitoli del video</h3>
          {loadingRows ? (
            <div className="flex items-center gap-2 text-muted-foreground text-sm py-4"><Loader2 size={16} className="animate-spin" /> Caricamento…</div>
          ) : righe.length === 0 ? (
            ...
            <div className="space-y-2">
              {righe.map((r) => {
                ...
                return (
                  <div key={r.id} className="flex items-center gap-3 bg-surface-1 border border-hairline rounded-2xl px-4 py-3">
                    <Badge variant="outline" className={`gap-1.5 px-2.5 py-1 ${st.border} ${st.bg} ${st.color} text-xs`}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {r.stato === "errore" && r.errore_msg && <p className="text-sm text-red-400/80 truncate" title={r.errore_msg}>{r.errore_msg}</p>}
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <Button variant="ghost" size="icon" onClick={() => handleRigenera(r.id)} title="Rimetti in coda" className="hover:bg-amber-500 hover:text-white bg-surface-3 text-muted-foreground w-8 h-8 rounded-xl"><RotateCw size={15} /></Button>
        ...
        <div className="space-y-5 lg:border-l lg:border-hairline lg:pl-8">
          <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground">Genera nuovi sottotitoli</h3>
          <div>
            <label className="block text-xs font-bold text-muted-foreground mb-2">Lingua parlata nel video</label>
            <div className="flex flex-wrap gap-2">
              <Button variant={source === "auto" ? "default" : "outline"} onClick={() => setSource("auto")} className={`rounded-xl text-xs font-bold ${source !== "auto" ? "bg-surface-2 border-hairline" : ""}`}>
                ✨ Auto (rileva)
              </Button>
              {LINGUE.map((l) => (
                <Button key={l.code} variant={source === l.code ? "default" : "outline"} onClick={() => setSource(l.code)} className={`rounded-xl text-xs font-bold ${source !== l.code ? "bg-surface-2 border-hairline" : ""}`}>
                  {l.label}
                </Button>
              ))}
            </div>
              ...
              {LINGUE.map((l) => {
                ...
                return (
                  <Button
                    ...
                    className={`rounded-xl text-xs font-bold flex items-center gap-1.5 ${active ? "bg-emerald-500/15 text-emerald-300 border-emerald-500/40" : "bg-surface-2 border-hairline"}`}
                  >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <span className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${active ? "bg-primary/20 text-primary" : "bg-surface-3 text-muted-foreground"}`}>
                        <Cpu size={15} />
                      </span>
                      <span className={`font-black text-sm ${active ? "text-foreground" : "text-foreground"}`}>{m.titolo}</span>
                      {active && <Check size={15} className="ml-auto text-primary" />}
                    </div>
                    <p className="text-sm text-muted-foreground font-semibold mt-1.5">{m.desc}</p>
                    <p className="text-xs text-muted-foreground uppercase tracking-wider font-bold mt-0.5">{m.hint}</p>
                  </button>
                );
              })}
            </div>
      ...
      <div className="px-6 py-4 flex gap-3 border-t border-hairline bg-surface-2 shrink-0 justify-end">
...
const STEP_COLORS = {
  ...
  pending: { ring: "border-hairline-strong bg-surface-2 text-muted-foreground", label: "text-muted-foreground", sub: "text-muted-foreground" },
...
function FlowStep({ Icon, label, sub, status, isFirst, isLast, lineInDone, lineOutDone }) {
  ...
  const lineColor = (filled) => (filled ? "bg-emerald-500/40" : "bg-surface-3");
  return (
    <div className="flex flex-col items-center flex-1 min-w-0">
      ...
      <p className={`mt-2 text-xs font-black uppercase tracking-wider text-center ${c.label}`}>{label}</p>
      {sub && <p className={`text-xs font-bold text-center truncate max-w-[100px] ${c.sub}`}>{sub}</p>}
    </div>
  );
}
...
function QueueCard({ rows, reload }) {
  ...
  return (
    <Card className="p-5 bg-surface-2 border-hairline space-y-5 rounded-3xl">
      <div className="flex items-center gap-5">
        <div className="group relative w-28 aspect-video rounded-xl overflow-hidden bg-background ring-1 ring-hairline-strong flex-shrink-0">
          <CoverThumb video={v} />
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            <Badge variant="outline" className="text-xs uppercase tracking-wider bg-surface-3 border-hairline px-2.5 py-1">
      ...
      {(() => {
        ...
        return (
          <div className="flex items-start bg-surface-3 rounded-2xl px-4 py-5 border border-hairline">
            {steps.map((s, i) => (
              <FlowStep
                key={s.key}
                Icon={s.Icon}
        ...
        {rows.map((r) => {
          ...
          return (
            <Badge key={r.id} variant="outline" className={`gap-1.5 px-2.5 py-1.5 rounded-xl ${st.border} ${st.bg} ${st.color}`}>
              ...
              <span className="text-xs uppercase tracking-wide opacity-70">{r.tipo}</span>
              {r.stato === "errore" && (
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        <Card className="p-4 bg-surface-2 border-hairline flex-row items-center gap-4 rounded-3xl">
          ...
          <div className="text-left min-w-0">
            ...
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mt-1">In lavorazione</p>
          </div>
        </Card>
        <Card className="p-4 bg-surface-2 border-hairline flex-row items-center gap-4 rounded-3xl">
          ...
          <div className="text-left min-w-0">
            ...
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mt-1">Video coinvolti</p>
          </div>
        </Card>
        <Card className="p-4 bg-surface-2 border-hairline flex-row items-center gap-4 rounded-3xl">
          ...
          <div className="text-left min-w-0">
            ...
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mt-1">Errori</p>
          </div>
        </Card>
      </div>
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2].map((i) => <Card key={i} className="h-44 bg-surface-2 rounded-3xl animate-pulse border-hairline" />)}
        </div>
      ) : groups.length === 0 ? (
        <Card className="flex flex-col items-center justify-center py-24 bg-transparent border-hairline-strong/50 border-dashed rounded-3xl">
          <Inbox size={48} className="text-muted-foreground mb-4" />
          <p className="text-muted-foreground font-bold">La coda è vuota.</p>
          <p className="text-muted-foreground text-sm mt-1">Accoda una generazione dalla Libreria per vederla qui.</p>
        </Card>
      ) : (
        <div className="space-y-4">
