    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="h-40 bg-zinc-900/50 rounded-3xl animate-pulse"
          ></div>
        ))}
      </div>
    );
    ...
    return (
      <div className="admin-card border-red-500/20 bg-red-500/5 flex flex-col items-center py-12">
        ...
        <p className="text-red-500 font-bold">
          Errore caricamento dati server.
        </p>
        <button
          onClick={() => window.location.reload()}
          className="mt-4 px-6 py-2 bg-red-500 text-white rounded-full font-bold text-sm"
        >
          Riprova
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="admin-card group">
          <div className="flex items-start justify-between mb-6">
            ...
            <div className="text-right">
              ...
              <h3 className="text-2xl font-black text-white mt-1">
                {stats.disco_usato_gb}{" "}
                <span className="text-xs text-zinc-500 font-bold">GB</span>
              </h3>
            </div>
          </div>
          ...
          <div className="w-full bg-zinc-800/50 h-2 rounded-full overflow-hidden mb-3">
            <div
              className={`h-full ${diskColor} shadow-[0_0_10px_rgba(var(--primary-rgb),0.5)] transition-all duration-1000 ease-out`}
              style={{ width: `${stats.disco_percentuale}%` }}
            ></div>
          </div>
        ...
        <div className="admin-card group">
          ...
          <p
            className="text-[10px] text-zinc-500 font-bold truncate bg-zinc-950/40 p-2 rounded-xl border border-white/5"
            ...
            >
              <div className="flex-1">
                ...
                <p className="text-sm font-medium text-zinc-400 mb-6">
                  Questo è il colore base che apparirà nella pagina di Login e
                  verrà applicato agli utenti che non scelgono un colore
                  personalizzato.
                </p>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-4 bg-zinc-900 border border-zinc-800 rounded-2xl p-4">
                  <div
                    className="relative w-16 h-16 rounded-xl overflow-hidden shrink-0 shadow-lg"
                    style={{
                      backgroundColor: globalTheme,
                      boxShadow: `0 0 20px ${globalTheme}40`,
                    }}
                  >
                    <input
                      type="color"
                  ...
                  <div className="flex-1 w-full text-center sm:text-left">
                    <span className="block text-xs font-black uppercase text-zinc-500 tracking-wider mb-1">
                      Codice Esadecimale
                    </span>
                    <input
                      type="text"
