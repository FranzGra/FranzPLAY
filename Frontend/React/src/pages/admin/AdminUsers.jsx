...
import { getAssetUrl, getUserAvatarUrl } from "../../services/helpers";
import {
  Trash2,
  Shield,
  ShieldOff,
...
export default function AdminUsers() {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        showNotification(
          res.message || "Impossibile ripristinare la password.",
          "error",
        );
      }
    } catch (error) {
      ...
      showNotification(
        "Si è verificato un errore critico durante l'operazione.",
        "error",
      );
    } finally {
  ...
  const handleToggleAdmin = async (id, currentStatus) => {
    ...
    if (
      !window.confirm(`Sei sicuro di voler ${actionName} i privilegi di Admin?`)
    )
      return;
    ...
    try {
      ...
      showNotification(
        `Permessi ${actionName === "rimuovere" ? "rimossi" : "concessi"} con successo`,
      );
    } catch (error) {
      showNotification("Errore: " + error.message, "error");
    }
  ...
  const handleDeleteUser = async (id) => {
    ...
    if (
      !window.confirm(
        "Attenzione: Questa azione eliminerà permanentemente l'utente e tutti i suoi dati. Continuare?",
      )
    )
      return;
        ...
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            <div
              ...
              onClick={() => !isResetting && setResetPasswordUserId(null)}
            ></div>
            <div className="relative w-full max-w-md bg-zinc-950 border border-white/10 rounded-3xl p-6 sm:p-8 shadow-2xl anim-card">
              ...
              <form onSubmit={handleResetPassword} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-zinc-500 mb-1.5 uppercase tracking-wider ml-1">
                    Nuova Password
                  </label>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-zinc-500 font-medium mt-1">
              Amministra i permessi e gli accessi alla piattaforma.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {loading ? (
            [1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-48 bg-zinc-900/50 rounded-3xl animate-pulse"
              ></div>
            ))
          ) : users.length === 0 ? (
            ...
            users.map((u) => {
              ...
              return (
                <div
                  key={u.id}
                  className="admin-card group relative overflow-hidden flex flex-col justify-between hover:bg-zinc-900/60"
                >
                  ...
                  <div>
                    <div className="flex items-start justify-between mb-6 relative z-10">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-14 h-14 rounded-2xl flex items-center justify-center overflow-hidden transition-all duration-300 ${isAdmin ? "bg-primary text-white" : "bg-zinc-800 text-zinc-500 group-hover:bg-zinc-700"}`}
                        >
                          {u.Immagine_Profilo ? (
                            <img
                              src={
                                u.Immagine_Profilo.startsWith("http")
                                  ? u.Immagine_Profilo
                                  : `${getUserAvatarUrl(u.Immagine_Profilo)}?t=${Date.now()}`
                              }
                              alt={u.Nome_Utente}
                        ...
                        <div>
                          <h3 className="font-bold text-lg text-white truncate max-w-[120px]">
                            {u.Nome_Utente}
                          </h3>
                  ...
                  {!isMe && (
                    <div className="flex items-center gap-2 pt-4 border-t border-white/5 relative z-10">
                      ...
                      >
                        {isAdmin ? (
                          <ShieldOff size={18} className="md:w-3.5 md:h-3.5" />
                        ) : (
                          <Shield size={18} className="md:w-3.5 md:h-3.5" />
                        )}
                  ...
                  {isMe && (
                    <div className="pt-4 border-t border-white/5 flex items-center justify-center relative z-10">
                      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-700 tabular-nums italic">
                        Il tuo Profilo
                      </span>
                    </div>
                  )}
